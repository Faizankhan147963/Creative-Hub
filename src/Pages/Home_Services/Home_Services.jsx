    ...
    <div className="min-h-screen bg-white container mx-auto md:py-8 md:px-0 px-4">
      ...
      <div className="mb-20 text-center">
        ...
        <h1 className="mb-6 text-3xl font-bold md:text-4xl lg:text-5xl">
          ...
          <br />
          services to <span className="inline-block rounded-full bg-[#7FFF00] px-6 py-2">the clients till their</span> satisfaction
        </h1>
          ...
          {services.map((service, index) => (
            ...
            to={`/service/${service.title}`}
              ...
              className={`group rounded-3xl ${service.bgColor} p-8 transition-all border border-gray-500${
                service.bgColor === "bg-[#7FFF00]" ? "text-black" : "text-gray-800"
              }`}
            >
              <div className="mb-6">{service.icon}</div>
              <h3 className="mb-4 text-xl font-bold">{service.title}</h3>
              <p className={service.bgColor === "bg-[#7FFF00]" ? "text-gray-800" : "text-gray-600"}>
                {service.description}
              </p>
            </Link>
          ))}
