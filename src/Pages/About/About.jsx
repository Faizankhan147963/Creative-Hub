      ...
      <div className="min-h-screen bg-white container mx-auto py-16 md:px-0 px-4">
        {/* Dark Section */}
          ...
          <div className="mb-8 flex items-center gap-2 text-sm">
            <Link to={'/'}> <span className="text-[#7FFF00]">HOME</span></Link>
          ...
          <div className="mb-12 flex items-start justify-between">
            ...
            <button className="rounded-full bg-[#7FFF00] px-6 py-3 font-semibold md:block hidden">👋 We are hiring</button>
          </div>
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {team.map((member, index) => (
              <div key={index} className="text-center">
                ...
                <p className="mb-4 text-[#7FFF00]">{member.role}</p>
                <div className="flex justify-center gap-4">
                  {["facebook", "twitter", "instagram"].map((social) => (
                    <a
                      key={social}
                      href={`#${social}`}
                      className="rounded-full bg-white/10 p-2 transition-colors hover:bg-[#7FFF00]"
                    >
